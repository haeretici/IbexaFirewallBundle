...
namespace Haeretici\FirewallBundle\Controller;
...
use Haeretici\FirewallBundle\Form\FirewallSettingsType;
...
class AdminController extends Controller
{
    ...
    public function performAccessCheck(): void
    {
        ...
        $this->denyAccessUnlessGranted(new Attribute('haeretici_firewall', 'admin'));
    }
    ...
    public function dashboardAction()
    {
        ...
        $metricsCacheItem = $this->cache->getItem('haeretici_server_metrics');
        ...
        return $this->render('@ibexadesign/haeretici/pages/dashboard.html.twig', $params);
    }
    ...
    public function settingsAction(Request $request): Response
    {
        ...
        if ($form->isSubmitted() && $form->isValid()) {
            try {
                ...
                $this->notificationHandler->success(
                    /** @Desc("Settings saved successfully. Some changes may require a page reload or server restart to take effect.") */
                    'settings.update.success',
                    [],
                    'haeretici_firewall'
                );

                return new RedirectResponse($this->generateUrl('haeretici_firewall.settings'));
            } catch (\Throwable $e) {
                $this->notificationHandler->error(
                    /** @Desc("Settings could not be saved. Please try again.") */
            ...
            $this->notificationHandler->error(
                /** @Desc("Could not save settings: %details%") */
                'settings.update.validation_error',
                ['%details%' => $this->formatFormErrors($form)],
                'haeretici_firewall'
            );
        }
        ...
        $params = [
            ...
            'breadcrumbs' => [
                ['value' => 'Firewall', 'url' => $this->generateUrl('haeretici_firewall.dashboard')],
                ['value' => 'Settings'],
            ],
            'form' => $form->createView(),
        ];

        return $this->render('@ibexadesign/haeretici/pages/settings.html.twig', $params);
    }
    ...
    private function formatFormErrors(FormInterface $form): string
    {
        ...
        foreach ($form->getErrors(true) as $error) {
            ...
            if ($label && $label !== '_token') {
                $messages[] = sprintf('%s: %s', $label, $message);
            } else {
                $messages[] = $message;
            }
        }

        return $messages !== [] ? implode('; ', array_unique($messages)) : 'Please check the highlighted fields.';
    }
    ...
    private function flattenConfig(array $config): array
    {
        return [
            ...
            'rate_limiting_min_response_time' => $config['rate_limiting']['min_response_time'],
            ...
            'exemptions_paths' => $this->pathsToText($config['exemptions']['paths'] ?? []),
            'honeypot_enabled' => $config['honeypot']['enabled'],
            'honeypot_ban_duration' => $config['honeypot']['ban_duration'],
            'honeypot_paths' => $this->pathsToText($config['honeypot']['paths'] ?? []),
        ];
    }

    private function pathsToText(array $paths): string
    {
        return implode("\n", array_map('strval', $paths));
    }

    private function textToPaths(?string $value): array
    {
        ...
        return array_values(array_filter(array_map('trim', $lines), static fn (string $path) => $path !== ''));
    }

    private function unflattenConfig(array $flat): array
    {
        $exemptionsPaths = $this->textToPaths($flat['exemptions_paths'] ?? '');
        $honeypotPaths = $this->textToPaths($flat['honeypot_paths'] ?? '');
        return [
            'rate_limiting' => [
                'window' => $flat['rate_limiting_window'],
                'max_requests' => $flat['rate_limiting_max_requests'],
                'min_response_time' => $flat['rate_limiting_min_response_time'],
            ...
            'exemptions' => [
                'paths' => $exemptionsPaths,
            ],
            'enable_rate_limiting' => $flat['enable_rate_limiting'],
            'honeypot' => [
                'enabled' => $flat['honeypot_enabled'],
                'ban_duration' => $flat['honeypot_ban_duration'],
                'paths' => $honeypotPaths,
            ],
        ];
    }
