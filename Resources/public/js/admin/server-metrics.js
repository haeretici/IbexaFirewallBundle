...
export function initServerMetricsWidget(container) {
    // Chart.js is loaded globally by Ibexa admin UI (chart.umd.js)
    const Chart = window.Chart;
    if (!Chart) {
        console.error('Chart.js is not available. Ensure Ibexa admin UI assets are installed.');
        container.innerHTML = '<div class="alert alert-warning">Chart.js is required for server metrics.</div>';
        return;
    }
    ...
    function loadData(range, start, end) {
        ...
        fetch(`${window.haeretici_root}/haeretici_firewall/metrics?${params}`)
